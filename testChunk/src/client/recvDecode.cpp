...
bool RecvDecode::recvFileHead(Recipe_t &fileRecipe, u_char *fileNameHash) {
    ...
    while (true) {
        ...
        if (respond.messageType == ERROR_FILE_NOT_EXIST) {
            cerr << "RecvDecode : Server reject download request, file not exist in "
                    "server!"
                 << endl;
            exit(1);
        }
        if (respond.messageType == ERROR_CHUNK_NOT_EXIST) {
            cerr << "RecvDecode : Server reject download request, chunk not exist in "
                    "server!"
                 << endl;
            exit(1);
        }
    ...
    while (totalRecvChunks < fileRecipe_.fileRecipeHead.totalChunkNumber) {
        ...
        if (respond.messageType == SUCCESS) {
            ...
            for (int i = 0; i < chunkNumber; i++) {
                ...
                totalRecvSize += sizeof(int);
                cryptoObj_->decryptChunk(respondBuffer + sizeof(NetworkHeadStruct_t) + totalRecvSize, chunkSize,
                                         respondBuffer + sizeof(NetworkHeadStruct_t) + totalRecvSize + chunkSize,
                                         chunkPlaintData);
