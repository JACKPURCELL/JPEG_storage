...
StorageCore::StorageCore() {
    outPutMQ_ = new messageQueue<Chunk_t>(config.get_RetrieverData_t_MQSize());
...
bool StorageCore::saveChunks(NetworkHeadStruct_t &networkHead, char *data) {
    ...
    // long diff = 1000000 * (timeendStorage.tv_sec - timestartStorage.tv_sec) + timeendStorage.tv_usec -
    // timestartStorage.tv_usec; double second = diff / 1000000.0; printf("save chunk list time is %ld us = %lf s\n",
    // diff, second);
    return true;
}
...
bool StorageCore::restoreRecipeHead(char * fileNameHash, Recipe_t &restoreRecipe) {
    ...
    if (fileName2metaDB.query(DBKey, recipeName)) {
        ...
        if (!RecipeIn.is_open()) {
            ...
            // cerr << "StorageCore : restore file size = " << restoreRecipe.fileRecipeHead.fileSize << " total chunk
            // number = " << restoreRecipe.fileRecipeHead.totalChunkNumber << endl;
...
bool StorageCore::saveRecipe(std::string recipeName, Recipe_t recipeHead, RecipeList_t recipeList, bool status) {
    ...
    if (status) {
        ...
        cerr << "StorageCore : save recipe head over, total chunk number = "
             << recipeHead.fileRecipeHead.totalChunkNumber << " file size = " << recipeHead.fileRecipeHead.fileSize
             << endl;
...
bool StorageCore::restoreRecipeAndChunk(char *fileNameHash, uint32_t startID, uint32_t endID,
        ...
        for (int i = 0; i < (endID - startID); i++) {
            ...
            if (restoreChunk(chunkHash, chunkData)) {
                if (chunkData.length() != newRecipeEntry.chunkSize) {
                    ...
                    if (!insertMQToRetriever(newChunk)) {
                    ...
                    // cout << "StorageCore : Restore Chunk ID = " << newChunk.ID << " size = " <<
                    // newChunk.logicDataSize << endl; PRINT_BYTE_ARRAY_STORAGE_CORE(stdout, newChunk.chunkHash,
                    // CHUNK_HASH_SIZE); PRINT_BYTE_ARRAY_STORAGE_CORE(stdout, newChunk.encryptKey,
                    // CHUNK_ENCRYPT_KEY_SIZE); PRINT_BYTE_ARRAY_STORAGE_CORE(stdout, newChunk.logicData,
                    // newChunk.logicDataSize);
                }
...
bool StorageCore::checkRecipeStatus(Recipe_t recipeHead, RecipeList_t recipeList) {
    ...
    // cerr << "StorageCore : recv recpie head, file size = " << recipeHead.fileRecipeHead.fileSize << " total chunk
    // number = " << recipeHead.fileRecipeHead.totalChunkNumber << endl; PRINT_BYTE_ARRAY_STORAGE_CORE(stderr,
    // recipeHead.fileRecipeHead.fileNameHash, FILE_NAME_HASH_SIZE);
    if (fileName2metaDB.query(DBKey, recipeName)) {
...
bool StorageCore::insertMQToRetriever(Chunk_t& newData)
{
    return outPutMQ_->push(newData);
}
bool StorageCore::extractMQToRetriever(Chunk_t& newData)
{
