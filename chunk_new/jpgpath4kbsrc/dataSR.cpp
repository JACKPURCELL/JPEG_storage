...
//
//void PRINT_BYTE_ARRAY_DATA_SR(FILE *file, void *mem, uint32_t len) {
//    if (!mem || !len) {
//        fprintf(file, "\n( null )\n");
//        return;
//    }
...
//}
//
//DataSR::DataSR(StorageCore *storageObj_) {
//    restoreChunkBatchSize = config.getSendChunkBatchSize();
//    storageObj_ = storageObj_;
//}
//
...
//            cerr << "DataSR : recv message type " << netBody.messageType << ", message size = " << netBody.dataSize
//                 << endl;
...
//                        memcpy(&tempRecipeEntry,
//                               recvBuffer + sizeof(NetworkHeadStruct_t) + sizeof(Recipe_t) + i * sizeof(RecipeEntry_t),
...
//
//                    if (storageObj_->restoreRecipeHead((char *)recvBuffer + sizeof(NetworkHeadStruct_t),
//                                                       restoredFileRecipe)) {
//                        cerr << "StorageCore : restore file size = " << restoredFileRecipe.fileRecipeHead.fileSize
//                             << " chunk number = " << restoredFileRecipe.fileRecipeHead.totalChunkNumber << endl;
//                        netBody.messageType = SUCCESS;
...
//                        if (storageObj_->restoreRecipeAndChunk((char *)recvBuffer + sizeof(NetworkHeadStruct_t),
//                                                               startID, endID, restoredChunkList)) {
//                            netBody.messageType = SUCCESS;
...
//                                memcpy(sendBuffer + sizeof(NetworkHeadStruct_t) + totalSendSize,
//                                       &restoredChunkList[i].ID, sizeof(uint32_t));
//                                totalSendSize += sizeof(uint32_t);
//                                memcpy(sendBuffer + sizeof(NetworkHeadStruct_t) + totalSendSize,
//                                       &restoredChunkList[i].logicDataSize, sizeof(int));
//                                totalSendSize += sizeof(int);
//                                memcpy(sendBuffer + sizeof(NetworkHeadStruct_t) + totalSendSize,
//                                       &restoredChunkList[i].logicData, restoredChunkList[i].logicDataSize);
//                                totalSendSize += restoredChunkList[i].logicDataSize;
//                                memcpy(sendBuffer + sizeof(NetworkHeadStruct_t) + totalSendSize,
//                                       &restoredChunkList[i].encryptKey, CHUNK_ENCRYPT_KEY_SIZE);
...
//                            startID = endID;
//                            if (restoredFileRecipe.fileRecipeHead.totalChunkNumber - totalRestoredChunkNumber <
//                                restoreChunkBatchSize) {
